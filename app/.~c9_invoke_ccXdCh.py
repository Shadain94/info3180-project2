...
def apiregister():
    ...
    if confirm_pass==password:
        ...
        db.session.add(user)
        response = jsonify(token=token,information={"error":"null","data":{'token':token,'expires': timeinfo(payload['exp']),'user':{'id':user.id,'email': user.email_address,'firstname':user.firstname,'lastname':user.lasttname'f':user.firstname},"message":"Success"}})
